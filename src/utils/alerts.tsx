const errorHandler = (result, c) => {
  if(!result.success) {
    const id = parseInt(c.req.param().id);
    const errors = [];    
    const errorAlert = ({ path, message }) => <div id='alert' class='alert alert-error animate-in fade-in-0 animate-duration-1000' _='on load or click wait 5s then remove me end on click remove me'>
       <svg
      ...
      <div>
        <h3 class="font-bold">Error in {path}</h3>
        <div class="text-xs">{message}</div>
      </div>      
    </div>    
    result.error.errors.forEach((error) => {
      errors.push(errorAlert(error))
    })
